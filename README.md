...
## Console application
This application simulates the situation of receiving data from a map about the location of the target in geodetic coordinates. The coordinates of the target are transferred from geodesics to Cartesian coordinates, and five test receivers are randomly placed in accordance with the scaling vector, and the time delays of the signal arrival are calculated. The difference-ranging method is applied, and target coordinates and model quality metrics are calculated. Finally, the Cartesian coordinates of the target are translated back into geodesics.  
Run ***RDM_CONSOLE.exe***  
